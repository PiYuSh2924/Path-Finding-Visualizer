  ...
  while (untraversedTiles.length > 0) {
    ...
    if (currentTile) {
      ...
      for (let i = 0; i < neighbours.length; i += 1) {
        ...
        if (distanceToNeighbor < neighbours[i].distance) {
          // Check if a shorter path is found
          dropFromQueue(neighbours[i], untraversedTiles); // Remove the neighbor from the queue
          neighbours[i].distance = distanceToNeighbor; // Update the neighbor's distance
          functionCost[neighbours[i].row][neighbours[i].col] =
            neighbours[i].distance +
            heuristicCost[neighbours[i].row][neighbours[i].col]; // Update the function cost for the neighbor
